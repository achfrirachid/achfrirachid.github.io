...
const CACHE_VERSION = 'v21-bg-parent-receipt-quicktext';
...
// ============================================================
// ⭐ إصلاح: إرسال "وصل الأداء" (نصي سريع) من الخلفية بمجرد رجوع النت،
// بلا ما يكون التطبيق مفتوح. الطابور (parentReceiptQueueX1) كان محفوظ
// فـ IndexedDB من قبل، بصح ماكانش حتى كود كيقراه من هنا — فكان كيبقى
// معلق حتى يتحل التطبيق يدويا. دابا كنستعملو مباشرة حساب Green API
// (idInstance/token) المخزن فـ cfg.whatsapp باش نصيفطو رسالة نصية فورية،
// والصورة (html2canvas) تبقى كتتصيفط من التطبيق ملي يتحل (بحال قبل).
// ============================================================
const PARENT_RECEIPT_QUEUE_IDB_KEY = 'parentReceiptQueueX1';
async function processParentReceiptQuickText(cfg){
  try{
    const queue = await cloudSyncIdbGet(PARENT_RECEIPT_QUEUE_IDB_KEY);
    if(!Array.isArray(queue) || !queue.length) return;
    if(!cfg || !cfg.whatsapp || !cfg.whatsapp.idInstance || !cfg.whatsapp.token) return;
    const myRole = cfg.isTeacher ? 'teacher' : 'admin';
    const remaining = [];
    for(const item of queue){
      if(!item || !item.digits) continue;
      // الطلب ديال حساب آخر (رشيد/عائشة) — نخليوه، غادي يتصيفط ملي هو يكون فالخلفية
      if(item.sender && item.sender !== myRole){ remaining.push(item); continue; }
      try{
        const text = '🧾 تم تسجيل أداء "' + (item.name || 'الطالب') + '" بنجاح ✅\nوصل الأداء غادي يوصل قريبا.';
        const base = 'https://api.green-api.com/waInstance' + cfg.whatsapp.idInstance;
        const res = await fetch(base + '/sendMessage/' + cfg.whatsapp.token, {
          method:'POST', headers:{'Content-Type':'application/json'},
          body: JSON.stringify({ chatId: item.digits + '@c.us', message: text })
        });
        const j = await res.json().catch(()=>null);
        if(!(res.ok && j && j.idMessage)) remaining.push(item); // فشل: يبقى فالطابور لمحاولة جاية
      }catch(e){ remaining.push(item); } // بلا نت أو خطأ: يبقى فالطابور
    }
    await cloudSyncIdbSet(PARENT_RECEIPT_QUEUE_IDB_KEY, remaining);
  }catch(e){ /* صامت — هادي مهمة مساعدة، ماخاصهاش توقف باقي المزامنة */ }
}

async function backgroundCloudSync(){
  const cfg = await cloudSyncIdbGet(K_C14);
  if(!cfg || !cfg.profileKey) return;
  await processParentReceiptQuickText(cfg);
  ...
  if(cfg.supabase && cfg.supabase.uid){
    // ⭐ فيكس 2026-09-09: قبل كان .catch(()=>{}) كيبلع أي خطأ بصمت، فـ Background
    // Sync كان كيحسبها "نجحت" حتى لو فشلات فعليا (ماكانش كيعاود المحاولة). دابا
    // كنتأكدو من res.ok ونرمي خطأ حقيقي عند الفشل.
    jobs.push((async () => {
      const res = await fetch(`${cfg.supabase.url}/rest/v1/backup?on_conflict=uid`, {
        method: 'POST',
        headers: {
      ...
      if(!res.ok) throw new Error('supabase-bg-sync-failed-' + res.status);
    })());
  }
  ...
  if(cfg.github && cfg.github.token && cfg.github.url){
    jobs.push((async () => {
      // ⭐ فيكس 2026-09-09: قبل كان catch(e){ /* صامت */ } كيبلع أي خطأ (بما فيهم
      // فشل الرفع نفسو، اللي ماكانش حتى كيتفحص عبر res.ok) — دابا كنتأكدو ونرمي.
      try{
        ...
        if(sha) body.sha = sha;
        const putRes = await fetch(cfg.github.url, {
          method: 'PUT',
          headers: Object.assign({ 'Content-Type': 'application/json' }, headers),
          body: JSON.stringify(body)
        });
        if(!putRes.ok) throw new Error('github-bg-sync-failed-' + putRes.status);
      }catch(e){ throw e; }
    })());
  }
  ...
  if(cfg.appwrite && cfg.appwrite.docUrl){
    jobs.push((async () => {
      // ⭐ فيكس 2026-09-09: نفس فيكس GitHub — كنتأكدو من res.ok ونرمي الخطأ.
      try{
        ...
        if(res.status === 404){
          res = await fetch(cfg.appwrite.collectionUrl, {
            method: 'POST', headers,
            body: JSON.stringify({ documentId: cfg.appwrite.docId, data: { data: jsonPretty }, permissions: ['read("any")', 'update("any")'] })
          });
        }
        if(!res.ok) throw new Error('appwrite-bg-sync-failed-' + res.status);
      }catch(e){ throw e; }
    })());
  }

  // ⭐ إضافة 2026-09-09: Firebase كان غايب تماما من المزامنة فالخلفية (كانت السحابة
  // الرئيسية "مصدر الحقيقة" ماكتخدمش والتطبيق مسكر). دابا زدناها — بنفس منطق البصمة
  // لكل طالب (id) اللي كيخدم بيه fbUploadBackup فالمقدمة، باش تصيفط غير التعديل/الطالب
  // الجديد، ماشي كل اللائحة. كتحتاج idToken طازة (مصيفط من التطبيق قبل ما يسكر —
  // صالح لمدة ساعة تقريبا؛ إلا فات الوقت وطاح التوكن، هاد الجزء غير غادي يفشل ويتعاود
  // المحاولة تلقائيا ملي يتفتح التطبيق من جديد — بلا ما يأثر على باقي السحابات).
  if(cfg.firebase && cfg.firebase.url && cfg.firebase.uid && cfg.firebase.idToken){
    jobs.push((async () => {
      const lastMap = cfg.firebase.lastPushedMap || {};
      const updates = {};
      ...
      for(const s of students){
        ...
        if(lastMap[s.id] !== fp){ updates['students/' + s.id] = s; changed++; }
      }
      for(const oldId in lastMap){
        if(!currentIds.has(String(oldId))){ updates['students/' + oldId] = null; changed++; }
      }
      if(changed === 0) return; // ماكاين حتى تعديل جديد من آخر مرة نجحات
      const res = await fetch(cfg.firebase.url + '/users/' + cfg.firebase.uid + '.json?auth=' + encodeURIComponent(cfg.firebase.idToken), {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(updates)
      });
      if(!res.ok) throw new Error('firebase-bg-sync-failed-' + res.status);
    })());
  }
  ...
  // ⭐ إضافة (بطلب المستخدم): رنة/تنبيه بمجرد ما تنجح المزامنة فالخلفية، حتى
  // والتطبيق مقفول بالكامل. الصفحة (index.html) عندها رنة "تي تي تي" مبنية
  // بـ AudioContext، بصح الـ Service Worker ماعندوش وصول لـ Audio API خالص —
  // فالبديل الوحيد الممكن هنا هو إشعار نظام حقيقي (self.registration.showNotification)
  // اللي كيصوت تلقائيا بصوت الإشعارات الافتراضي ديال الهاتف + اهتزاز، حتى
  // بلا ما يكون التطبيق مفتوح فالخلفية ولا فالمقدمة.
  if(jobs.length > 0 && self.registration && typeof self.registration.showNotification === 'function'){
    try{
      await self.registration.showNotification('✅ تمت المزامنة', {
        body: 'بيانات الطلبة تصيفطات لكل السحابات بنجاح.',
        tag: 'cloud-sync-success',
        renotify: true,
        vibrate: [90,70,90,70,90],
        icon: './icon-192.png',
        badge: './icon-192.png'
      });
    }catch(e){ /* الإشعار اختياري — ماخاصوش يوقف المزامنة إلا فشل */ }
  }
}
