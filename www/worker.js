...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // ============================================================
      // JSON Blob — سحابة خفيفة إضافية بلا توكن (jsonblob.com)
      // ⭐ خدمة مجانية بسيطة موجودة من سنين، بلا حساب وبلا تسجيل: كتبعت
      // ليها JSON وكترجع ليك ID، ومن بعد كتقدر تعاود PUT/GET بيه للأبد.
      // كنخزنو الـ ID ديال كل مستخدم (users/xxx/students.json) فنفس
      // STUDENTS_KV اللي خدام مع /kv-read و/kv-write، تحت مفتاح مختلف
      // (jsonblobId:<path>) باش ما يتخالطش مع الداتا الحقيقية.
      // ============================================================
      if (url.pathname === "/blob-write" && request.method === "POST") {
        if (!env.STUDENTS_KV) {
          return json({ error: "missing_kv_binding", message: "STUDENTS_KV KV binding ماشي معمر فـ wrangler.toml" }, 500, origin);
        }
        const body = await request.json(); // { path, data }
        if (!/^users\/[A-Za-z0-9_\-]+\/students\.json$/.test(body.path || "")) {
          return json({ error: "invalid_path" }, 400, origin);
        }
        if (!Array.isArray(body.data)) {
        ...
        const idKey = "jsonblobId:" + body.path;
        const payload = JSON.stringify(body.data);
        try {
          let blobId = await env.STUDENTS_KV.get(idKey);
          if (!blobId) {
            ...
            if (!createRes.ok) return json({ ok: false, error: "create-failed:" + createRes.status }, 502, origin);
            const loc = createRes.headers.get("Location") || "";
            blobId = loc.split("/").pop();
            if (!blobId) return json({ ok: false, error: "no-location-header" }, 502, origin);
            await env.STUDENTS_KV.put(idKey, blobId);
            return json({ ok: true }, 200, origin);
          }
          let putRes = await fetch(`https://jsonblob.com/api/jsonBlob/${blobId}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: payload,
          });
          if (putRes.status === 404) {
            // الـ blob تحذف من عند jsonblob.com (نادر) — نعاودو نخلقو وحد جديد
            await env.STUDENTS_KV.delete(idKey);
            const createRes = await fetch("https://jsonblob.com/api/jsonBlob", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: payload,
            });
            if (!createRes.ok) return json({ ok: false, error: "recreate-failed:" + createRes.status }, 502, origin);
            const loc = createRes.headers.get("Location") || "";
            blobId = loc.split("/").pop();
            if (!blobId) return json({ ok: false, error: "no-location-header" }, 502, origin);
            await env.STUDENTS_KV.put(idKey, blobId);
          ...
          return json({ ok: putRes.ok, error: putRes.ok ? null : "update-failed:" + putRes.status }, putRes.ok ? 200 : 502, origin);
        } catch (e) {
          return json({ ok: false, error: String(e) }, 500, origin);
        }
      }

      if (url.pathname === "/blob-read" && request.method === "GET") {
        if (!env.STUDENTS_KV) {
          return json({ error: "missing_kv_binding", message: "STUDENTS_KV KV binding ماشي معمر فـ wrangler.toml" }, 500, origin);
        }
        const path = url.searchParams.get("path");
        ...
        const idKey = "jsonblobId:" + path;
        try {
          const blobId = await env.STUDENTS_KV.get(idKey);
          if (!blobId) return json({ ok: true, data: null }, 200, origin);
          const r = await fetch(`https://jsonblob.com/api/jsonBlob/${blobId}`);
          if (!r.ok) return json({ ok: false, error: "read-failed:" + r.status }, 502, origin);
