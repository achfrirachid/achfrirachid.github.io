...
// ============================================================
// ⏰ تفعيل تلقائي (Cron Trigger) — نبضة دورية تفيق Supabase وAppwrite من
// "النعاس" (auto-pause بعد أسبوع تقريبا من قلة النشاط عند Supabase مثلا)،
// بلا ما تحتاج حتى حد يفتح التطبيق. كل سحابة عندها 4 طبقات محاولة (إلا
// فشلت الأولى، الثانية كتحاول بعدها بقليل، وهكذا للرابعة) باش ما تفوتش
// النبضة بسبب مشكل شبكة عابر فقط. كل سحابة معزولة (Promise.allSettled) —
// توقف وحدة ما يأخرش ولا يوقف محاولة الأخرى. مفعّلة عبر Cron Trigger فـ
// Cloudflare (Worker → Settings → Triggers → Cron Triggers)، خاصها هاد
// الـ env vars: SUPABASE_URL, SUPABASE_ANON_KEY, APPWRITE_ENDPOINT,
// APPWRITE_PROJECT_ID, APPWRITE_DATABASE_ID (و APPWRITE_COLLECTION_ID
// اختياري، افتراضيا "backup").
// ============================================================
async function pingWithRetry(name, fn, maxAttempts = 4) {
  let lastErr = null;
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      await fn();
      console.log(`[keepalive] ${name} — نجحت (محاولة ${attempt}/${maxAttempts})`);
      return true;
    } catch (e) {
      lastErr = e;
      console.error(`[keepalive] ${name} — فشلت المحاولة ${attempt}/${maxAttempts}: ${e.message || e}`);
      if (attempt < maxAttempts) await new Promise((r) => setTimeout(r, attempt * 2000));
    }
  }
  console.error(`[keepalive] ${name} — فشلت الطبقات الأربعة كاملين. آخر خطأ: ${lastErr?.message || lastErr}`);
  return false;
}

async function pingSupabaseKeepAlive(env) {
  if (!env.SUPABASE_URL || !env.SUPABASE_ANON_KEY) throw new Error("missing_supabase_env");
  const r = await fetch(`${env.SUPABASE_URL}/rest/v1/backup?select=uid&limit=1`, {
    headers: { apikey: env.SUPABASE_ANON_KEY, Authorization: `Bearer ${env.SUPABASE_ANON_KEY}` },
  });
  if (!r.ok) throw new Error("supabase_http_" + r.status);
}

async function pingAppwriteKeepAlive(env) {
  if (!env.APPWRITE_ENDPOINT || !env.APPWRITE_PROJECT_ID || !env.APPWRITE_DATABASE_ID) throw new Error("missing_appwrite_env");
  const collection = env.APPWRITE_COLLECTION_ID || "backup";
  const r = await fetch(
    `${env.APPWRITE_ENDPOINT}/databases/${env.APPWRITE_DATABASE_ID}/collections/${collection}/documents?queries[0]=${encodeURIComponent("limit(1)")}`,
    { headers: { "X-Appwrite-Project": env.APPWRITE_PROJECT_ID } }
  );
  if (!r.ok && r.status !== 404) throw new Error("appwrite_http_" + r.status);
}

async function runKeepAlivePings(env) {
  await Promise.allSettled([
    pingWithRetry("Supabase", () => pingSupabaseKeepAlive(env)),
    pingWithRetry("Appwrite", () => pingAppwriteKeepAlive(env)),
  ]);
}
...
export default {
  ...
  // ⭐ كيتشغل تلقائيا بالـ Cron Trigger المضبوط فـ Cloudflare Dashboard —
  // بلا أي علاقة بطلبات المستخدم العادية (fetch فوق). ctx.waitUntil كيضمن
  // الـ Worker ما يوقفش قبل ما تكمل النبضات، حتى ولو خدات بضع ثواني.
  async scheduled(event, env, ctx) {
    ctx.waitUntil(runKeepAlivePings(env));
  },
};
