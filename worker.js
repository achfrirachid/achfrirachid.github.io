...
// kDrive (Infomaniak) عبر WebDAV — Basic Auth بإيميل + app password (كتصنع من
// manager.infomaniak.com > الملف الشخصي > الأمان > Application passwords)
function kdriveBaseUrl(env) {
  return `https://${env.KDRIVE_ID}.connect.kdrive.infomaniak.com`;
}
function kdriveAuthHeader(env) {
  return "Basic " + btoa(`${env.KDRIVE_EMAIL}:${env.KDRIVE_APP_PASSWORD}`);
}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // ---- Dropbox: رفع/قراءة (بيانات كاملة، ملف منفصل لكل دور) — refresh token يتجدد فـ كل طلب ----
      if (url.pathname === "/dropbox-upload" && request.method === "POST") {
        const { list } = await request.json();
        const role = url.searchParams.get("role") === "aicha" ? "aicha" : "rachid";
        const accessToken = await getDropboxAccessToken(env);
        const r = await fetch("https://content.dropboxapi.com/2/files/upload", {
          method: "POST",
          headers: {
            Authorization: `Bearer ${accessToken}`,
            "Dropbox-API-Arg": JSON.stringify({ path: `/tatabbou-backup-${role}.json`, mode: "overwrite", mute: true }),
      ...
      if (url.pathname === "/dropbox-fetch" && request.method === "GET") {
        ...
        const r = await fetch("https://content.dropboxapi.com/2/files/download", {
          ...
          headers: {
            Authorization: `Bearer ${accessToken}`,
            "Dropbox-API-Arg": JSON.stringify({ path: `/tatabbou-backup-${role}.json` }),
          },
        });
      ...
      // ---- kDrive (Infomaniak, WebDAV): رفع/قراءة (Light Cloud backup) — ملف منفصل لكل دور (رشيد/عائشة) ----
      if (url.pathname === "/kdrive-upload" && request.method === "POST") {
        const { list } = await request.json();
        const role = url.searchParams.get("role") === "aicha" ? "aicha" : "rachid";
        const r = await fetch(`${kdriveBaseUrl(env)}/tatabbou-backup-${role}.json`, {
          method: "PUT",
          headers: {
            Authorization: kdriveAuthHeader(env),
        ...
        if (!r.ok) {
          ...
          return json({ ok: false, kdrive_status: r.status, kdrive_body: bodyText.slice(0, 500) }, 502, origin);
        }
        return json({ ok: true }, 200, origin);
      }
      if (url.pathname === "/kdrive-fetch" && request.method === "GET") {
        const role = url.searchParams.get("role") === "aicha" ? "aicha" : "rachid";
        const r = await fetch(`${kdriveBaseUrl(env)}/tatabbou-backup-${role}.json`, {
          method: "GET",
          headers: { Authorization: kdriveAuthHeader(env) },
        });
        if (!r.ok) {
          const bodyText = await r.text().catch(() => "");
          return json({ error: "not_found", kdrive_status: r.status, kdrive_body: bodyText.slice(0, 500) }, 404, origin);
        }
        const text = await r.text();
        return json({ content: text }, 200, origin);
      }
