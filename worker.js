...
async function neonQuery(env, query, params) {
  if (!env.NEON_DATABASE_URL) throw new Error("neon_not_configured");
  const host = new URL(env.NEON_DATABASE_URL.replace(/^postgres(ql)?:\/\//, "https://")).host;
  ...
  if (!r.ok) {
    ...
    throw new Error("neon_query_failed_" + r.status + "_" + t.slice(0, 200));
  }
  return r.json();
}
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/d1-upload" && request.method === "POST") {
        if (!env.DB) return json({ error: "d1_binding_missing" }, 500, origin);
      ...
      if (url.pathname === "/kv-upload" && request.method === "POST") {
        if (!env.KV) return json({ error: "kv_binding_missing" }, 500, origin);
        const { uid, list } = await request.json();
        if (!uid) return json({ error: "missing_uid" }, 400, origin);
      ...
      // ---- Neon (Postgres): رفع/قراءة عبر HTTP driver (بلا حزمة npm) ----
      if (url.pathname === "/neon/upload" && request.method === "POST") {
        if (!env.NEON_DATABASE_URL) return json({ error: "neon_not_configured" }, 500, origin);
        const { uid, data } = await request.json();
        if (!uid) return json({ error: "missing_uid" }, 400, origin);
        await neonQuery(env, "CREATE TABLE IF NOT EXISTS backups (uid TEXT PRIMARY KEY, data TEXT, updated_at TEXT)", []);
        await neonQuery(
          env,
          "INSERT INTO backups (uid, data, updated_at) VALUES ($1, $2, $3) ON CONFLICT (uid) DO UPDATE SET data = EXCLUDED.data, updated_at = EXCLUDED.updated_at",
          [uid, JSON.stringify(data), new Date().toISOString()]
        );
        return json({ ok: true }, 200, origin);
      }
      if (url.pathname === "/neon/fetch" && request.method === "GET") {
        if (!env.NEON_DATABASE_URL) return json({ error: "neon_not_configured" }, 500, origin);
        const uid = url.searchParams.get("uid");
        if (!uid) return json({ error: "missing_uid" }, 400, origin);
        await neonQuery(env, "CREATE TABLE IF NOT EXISTS backups (uid TEXT PRIMARY KEY, data TEXT, updated_at TEXT)", []);
        const result = await neonQuery(env, "SELECT data FROM backups WHERE uid = $1", [uid]);
        const row = result && result.rows && result.rows[0];
        if (!row) return json({ error: "not_found" }, 404, origin);
        let parsed = null;
        try { parsed = JSON.parse(row.data); } catch (e) { parsed = null; }
        return json({ data: parsed }, 200, origin);
      }
