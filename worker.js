...
async function getOneDriveAccessToken(env) {
  const r = await fetch(`https://login.microsoftonline.com/${env.ONEDRIVE_TENANT || "common"}/oauth2/v2.0/token`, {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: new URLSearchParams({
      grant_type: "refresh_token",
  ...
  const j = await r.json();
  if (!r.ok || !j.access_token) throw new Error("onedrive_auth_failed");
  return j.access_token;
}

async function getBoxAccessToken(env) {
  ...
  const j = await r.json();
  if (!r.ok || !j.access_token) throw new Error("box_auth_failed");
  return j.access_token;
}
...
export default {
  async fetch(request, env) {
    ...
    // (onedrive-callback مستثنى لأنو كيتزار من المتصفح مباشرة بلا headers — مؤقت، يتمسح من بعد الإعداد)
    const key = request.headers.get("X-Proxy-Key") || "";
    if (url.pathname !== "/onedrive-callback" && (!env.PROXY_KEY || key !== env.PROXY_KEY)) {
    ...
    try {
      ...
      // ---- OneDrive: رفع/قراءة (Light Cloud backup) — refresh token يتجدد فـ كل طلب ----
      if (url.pathname === "/onedrive-upload" && request.method === "POST") {
        const { list } = await request.json();
        const accessToken = await getOneDriveAccessToken(env);
        const r = await fetch("https://graph.microsoft.com/v1.0/me/drive/root:/tatabbou-backup.json:/content", {
          method: "PUT",
      ...
      if (url.pathname === "/onedrive-fetch" && request.method === "GET") {
        const accessToken = await getOneDriveAccessToken(env);
        const r = await fetch("https://graph.microsoft.com/v1.0/me/drive/root:/tatabbou-backup.json:/content", {
      ...
      // ---- Box: رفع/قراءة (Light Cloud backup) — refresh token يتجدد فـ كل طلب ----
      if (url.pathname === "/box-upload" && request.method === "POST") {
        const { list } = await request.json();
        const accessToken = await getBoxAccessToken(env);
        const searchR = await fetch("https://api.box.com/2.0/search?query=tatabbou-backup.json&type=file", {
          headers: { Authorization: `Bearer ${accessToken}` },
        });
        const searchJ = await searchR.json().catch(() => null);
        const existingId = searchJ && searchJ.entries && searchJ.entries[0] ? searchJ.entries[0].id : null;
        const form = new FormData();
        form.append("attributes", JSON.stringify(existingId ? {} : { name: "tatabbou-backup.json", parent: { id: (env.BOX_FOLDER_ID || "0") } }));
        form.append("file", new Blob([JSON.stringify(list)], { type: "application/json" }), "tatabbou-backup.json");
        const uploadUrl = existingId
          ? `https://upload.box.com/api/2.0/files/${existingId}/content`
          : "https://upload.box.com/api/2.0/files/content";
        const r = await fetch(uploadUrl, { method: "POST", headers: { Authorization: `Bearer ${accessToken}` }, body: form });
        return json({ ok: r.ok }, r.ok ? 200 : 502, origin);
      }
      if (url.pathname === "/box-fetch" && request.method === "GET") {
        const accessToken = await getBoxAccessToken(env);
        const searchR = await fetch("https://api.box.com/2.0/search?query=tatabbou-backup.json&type=file", {
          headers: { Authorization: `Bearer ${accessToken}` },
        });
        const searchJ = await searchR.json().catch(() => null);
        const existingId = searchJ && searchJ.entries && searchJ.entries[0] ? searchJ.entries[0].id : null;
        if (!existingId) return json({ error: "not_found" }, 404, origin);
        const r = await fetch(`https://api.box.com/2.0/files/${existingId}/content`, {
          headers: { Authorization: `Bearer ${accessToken}` },
        });
        if (!r.ok) return json({ error: "not_found" }, 404, origin);
        const text = await r.text();
      ...
      // ---- 🛠️ مؤقت: إعداد OneDrive لمرة وحدة — بعد ما تدير الكود، امسحه ----
      if (url.pathname === "/onedrive-callback" && request.method === "GET") {
        ...
        const r = await fetch("https://login.microsoftonline.com/common/oauth2/v2.0/token", {
          ...
          body: new URLSearchParams({
            ...
            client_id: env.ONEDRIVE_CLIENT_ID,
            client_secret: env.ONEDRIVE_CLIENT_SECRET,
            redirect_uri: url.origin + "/onedrive-callback",
            scope: "Files.ReadWrite offline_access",
          }),
        });
        const j = await r.json();
        if (!r.ok) return json({ error: "exchange_failed", detail: j }, 400, origin);
        return json({ refresh_token: j.refresh_token }, 200, origin);
      }
